...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Gdrill.LightScryptN, Gdrill.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.drillum", new NodeConfig());
			node.start();
			...
			DrillumClient ec = node.getDrillumClient();
	...
	// signatures too. Regression test for go-drillum issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/drillum/gdrilltest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Gdrill and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.drillum", "github.com/drillum-network/go-drillum/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gdrill.aar"), "gdrill.aar")

	if err = os.WriteFile(filepath.Join("src", "androidTest", "java", "org", "drillum", "gdrilltest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.drillum.gdrilltest"
