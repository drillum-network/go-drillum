...
type ethNode struct {
	...
	ethBackend *eth.Drillum
	lapi       *lescatalyst.ConsensusAPI
	lesBackend *les.LightDrillum
}
...
func makeFullNode(genesis *core.Genesis) (*node.Node, *eth.Drillum, *ethcatalyst.ConsensusAPI, error) {
	// Define the basic configurations for the Drillum node
	...
	// Create the node and configure a full Drillum node on it
...
func makeLightNode(genesis *core.Genesis) (*node.Node, *les.LightDrillum, *lescatalyst.ConsensusAPI, error) {
	// Define the basic configurations for the Drillum node
	datadir, _ := os.MkdirTemp("", "")
