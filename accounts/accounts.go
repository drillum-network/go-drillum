...
// Package accounts implements high level Drillum account management.
package accounts
...
// Account represents an Drillum account located at a specific location defined
// by the optional URL field.
type Account struct {
	Address common.Address `json:"address"` // Drillum account address derived from the key
...
type Wallet interface {
	...
	// chain state reader.
	SelfDerive(bases []DerivationPath, chain drillum.ChainStateReader)
...
func TextAndHash(data []byte) ([]byte, string) {
	msg := fmt.Sprintf("\x19Drillum Signed Message:\n%d%s", len(data), string(data))
