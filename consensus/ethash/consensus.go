...
var (
	...
	DrillBlockReward, _           = big.NewInt(0).SetString("50000000000000000000", 10)
	maxUncles                     = 2         // Maximum number of uncles allowed in a single block
	allowedFutureBlockTimeSeconds = int64(15) // Max seconds from current time allowed for blocks, before they're considered future blocks

	calcDifficultyDrill = makeDifficultyCalculator(big.NewInt(15_000_000))
...
// rules of the stock Drillum ethash engine.
...
func CalcDifficulty(config *params.ChainConfig, time uint64, parent *types.Header) *big.Int {
	...
	switch {
	case config.IsDrill(next):
		return calcDifficultyDrill(time, parent)
...
func accumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, uncles []*types.Header) {
	...
	if config.IsDrill(header.Number) {
		blockReward = DrillBlockReward
	}
