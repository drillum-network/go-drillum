...
func (g *gdrillrpc) killAndWait() {
	g.gdrill.Kill()
	g.gdrill.WaitExit()
}

func (g *gdrillrpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.gdrill.Fatalf("callRPC %v: %v", method, err)
	}
}

func (g *gdrillrpc) addPeer(peer *gdrillrpc) {
	g.gdrill.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.gdrill.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.gdrill.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.gdrill.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.gdrill.Error("timeout adding peer after", dur)
	}
}

// Use this function instead of `g.nodeInfo` directly
func (g *gdrillrpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func startGdrillWithIpc(t *testing.T, name string, args ...string) *gdrillrpc {
	ipcName := fmt.Sprintf("gdrill-%d.ipc", atomic.AddUint32(&nextIPC, 1))
	args = append([]string{"--networkid=42", "--port=0", "--authrpc.port", "0", "--ipcpath", ipcName}, args...)
	t.Logf("Starting %v with rpc: %v", name, args)

	g := &gdrillrpc{
		name: name,
		gdrill: runGdrill(t, args...),
	}
	ipcpath := ipcEndpoint(ipcName, g.gdrill.Datadir)
	// We can't know exactly how long gdrill will take to start, so we try 10
...
func initGdrill(t *testing.T) string {
	args := []string{"--networkid=42", "init", "./testdata/clique.json"}
	t.Logf("Initializing gdrill: %v ", args)
	g := runGdrill(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
...
func startLightServer(t *testing.T) *gdrillrpc {
	datadir := initGdrill(t)
	t.Logf("Importing keys to gdrill")
	runGdrill(t, "account", "import", "--datadir", datadir, "--password", "./testdata/password.txt", "--lightkdf", "./testdata/key.prv").WaitExit()
	account := "0x02f0d131f1f97aef08aec6e3291b957d9efe7105"
	server := startGdrillWithIpc(t, "lightserver", "--allow-insecure-unlock", "--datadir", datadir, "--password", "./testdata/password.txt", "--unlock", account, "--mine", "--light.serve=100", "--light.maxpeers=1", "--nodiscover", "--nat=extip:127.0.0.1", "--verbosity=4")
	return server
}

func startClient(t *testing.T, name string) *gdrillrpc {
	datadir := initGdrill(t)
	return startGdrillWithIpc(t, name, "--datadir", datadir, "--nodiscover", "--syncmode=light", "--nat=extip:127.0.0.1", "--verbosity=4")
}
