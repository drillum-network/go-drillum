...
// gdrill is the official command-line client for Drillum.
package main
...
import (
	...
	"github.com/drillum-network/go-drillum/console/prompt"
	...
	_ "github.com/drillum-network/go-drillum/eth/tracers/js"
	_ "github.com/drillum-network/go-drillum/eth/tracers/native"

	"github.com/urfave/cli/v2"
)

const (
	clientIdentifier = "gdrill" // Client identifier to advertise over the network
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the go-drillum command line interface")
...
func init() {
	// Initialize the CLI app and start Gdrill
	app.Action = gdrill
	app.HideVersion = true // we have a command to print the version
	app.Copyright = "Copyright 2013-2022 The go-drillum Authors"
...
func prepare(ctx *cli.Context) {
	...
	case ctx.IsSet(utils.RopstenFlag.Name):
		log.Info("Starting Gdrill on Ropsten testnet...")

	case ctx.IsSet(utils.RinkebyFlag.Name):
		log.Info("Starting Gdrill on Rinkeby testnet...")

	case ctx.IsSet(utils.GoerliFlag.Name):
		log.Info("Starting Gdrill on Görli testnet...")

	case ctx.IsSet(utils.SepoliaFlag.Name):
		log.Info("Starting Gdrill on Sepolia testnet...")

	case ctx.IsSet(utils.KilnFlag.Name):
		log.Info("Starting Gdrill on Kiln testnet...")

	case ctx.IsSet(utils.DeveloperFlag.Name):
		log.Info("Starting Gdrill in ephemeral dev mode...")
		log.Warn(`You are running Gdrill in --dev mode. Please note the following:
...
`)
	...
	case !ctx.IsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting Gdrill on Drillum mainnet...")
	}
...
// gdrill is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func gdrill(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node, backend ethapi.Backend, isConsole bool) {
	...
	// Create a client to interact with local gdrill node.
	...
	if ctx.Bool(utils.MiningEnabledFlag.Name) || ctx.Bool(utils.DeveloperFlag.Name) {
		// Mining only makes sense if a full Drillum node is running
		if ctx.String(utils.SyncModeFlag.Name) == "light" {
			utils.Fatalf("Light clients do not support mining")
		}
		ethBackend, ok := backend.(*eth.EthAPIBackend)
		if !ok {
			utils.Fatalf("Drillum service not running")
		}
