...
import (
	...
	"github.com/drillum-network/go-drillum/accounts/external"
	"github.com/drillum-network/go-drillum/accounts/keystore"
	"github.com/drillum-network/go-drillum/accounts/scwallet"
	"github.com/drillum-network/go-drillum/accounts/usbwallet"
...
func loadConfig(file string, cfg *gdrillConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "gdrill.ipc"
	return cfg
}

// makeConfigNode loads gdrill configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, gdrillConfig) {
	// Load defaults.
	cfg := gdrillConfig{
...
// makeFullNode loads gdrill configuration and creates the Drillum backend.
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	if eth != nil && !ctx.IsSet(utils.IgnoreLegacyReceiptsFlag.Name) {
		...
		if err != nil {
			...
			stack.Close()
			utils.Fatalf("Database has receipts with a legacy format. Please run `gdrill db freezer-migrate`.")
		}
	}
	...
	// Add the Drillum Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
...
func applyMetricConfig(ctx *cli.Context, cfg *gdrillConfig) {
