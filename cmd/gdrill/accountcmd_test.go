...
func TestAccountListEmpty(t *testing.T) {
	gdrill := runGdrill(t, "account", "list")
	gdrill.ExpectExit()
}
...
`
	...
	{
		gdrill := runGdrill(t, "account", "list", "--datadir", datadir)
		gdrill.Expect(want)
		gdrill.ExpectExit()
	}
	{
		gdrill := runGdrill(t, "--datadir", datadir, "account", "list")
		gdrill.Expect(want)
		gdrill.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	gdrill := runGdrill(t, "account", "new", "--lightkdf")
	defer gdrill.ExpectExit()
	gdrill.Expect(`
...
func TestAccountHelp(t *testing.T) {
	gdrill := runGdrill(t, "account", "-h")
	gdrill.WaitExit()
	if have, want := gdrill.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	gdrill = runGdrill(t, "account", "import", "-h")
	gdrill.WaitExit()
	if have, want := gdrill.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gdrill := runGdrill(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer gdrill.ExpectExit()
	gdrill.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gdrill := runGdrill(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	gdrill := runGdrill(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gdrill.ExpectExit()
	gdrill.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := os.ReadDir(filepath.Join(gdrill.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gdrill := runMinimalGdrill(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gdrill.StderrText(), m) {
...
// https://github.com/drillum-network/go-drillum/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gdrill.SetTemplateFunc("keypath", func(file string) string {
