...
// information related to the Drillum protocol such als blocks, transactions and logs.
...
func (api *FilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(drillum.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as drillum.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria drillum.FilterQuery
...
func (api *FilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(drillum.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
