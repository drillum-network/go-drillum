...
// Package gdrillclient provides an RPC client for gdrill-specific APIs.
package gdrillclient
...
// Client is a wrapper around rpc.Client that implements gdrill-specific functionality.
//
// If you want to use the standardized Drillum RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg drillum.CallMsg) (*types.AccessList, uint64, string, error) {
...
func (ec *Client) CallContract(ctx context.Context, msg drillum.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
...
// GCStats retrieves the current garbage collection stats from a gdrill node.
...
// MemStats retrieves the current memory stats from a gdrill node.
...
// GetNodeInfo retrieves the node info of a gdrill node.
