...
const schema string = `
    ...
    scalar Bytes32
    # Address is a 20 byte Drillum address, represented as 0x-prefixed hexadecimal.
    scalar Address
    ...
    # Account is an Drillum account at a particular block.
    type Account {
    ...
    # Transaction is an Drillum transaction.
    type Transaction {
    ...
    type Block {
        ...
        # Account fetches an Drillum account at the current block's state.
    ...
    type Pending {
      ...
      transactions: [Transaction!]
      # Account fetches an Drillum account for the pending state.
      account(address: Address!): Account!
    ...
    type Query {
        # Block fetches an Drillum block by number or by hash. If neither is
